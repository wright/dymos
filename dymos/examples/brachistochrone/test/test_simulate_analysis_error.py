...
# Intentionally generate an AnalysisError to test display of additional information before error
class BrachistochroneAnalysisErrorODE(om.ExplicitComponent):
    def initialize(self):
    ...
    def setup(self):
        ...
        # Inputs
        self.add_input('y', val=np.zeros(nn), desc='height', units='m')
    ...
    def compute(self, inputs, outputs):
        ...
        if np.any(y < 5):
            raise om.AnalysisError('Invalid height detected')
...
class TestBrachistochroneQuickStart(unittest.TestCase):
    def test_brachistochrone_quick_start(self):
        ...
        phase = dm.Phase(ode_class=BrachistochroneAnalysisErrorODE,
        ...
        p.set_val('traj.phase0.t_duration', 10, units='s')
        ...
        p.set_val('traj.phase0.controls:theta',
                  phase.interpolate(ys=[45, 45], nodes='control_input'),
                  units='deg')

        # Run the model to initialize the problem
        p.run_model()
        #om.n2(p)

        # AnalysisError should be generated by simulate, with additional debugging output before it
        #self.assertRaises(AnalysisError, traj.simulate)
